...
void test_Game2D() {
    Game2D game("gopher_media", MakeRectangle(100, 100, 1024, 768), 60.0f);
    ...
    if (game.GetSuccess()) {
        while (game.GetIsRunning()) {
            game.Update();
            gopher_image.Render(MakeRectangle(0, 0, gopher_image.GetWidth() / 4, gopher_image.GetHeight() / 4), 10.0f, 
                                {gopher_image.GetWidth() / 8, gopher_image.GetHeight() / 8}, curr_flip);
            game.RenderRectangle(MakeRectangle(100, 100, 100, 200), {255, 0, 0, 100});
